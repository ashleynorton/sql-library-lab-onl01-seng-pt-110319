...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN authors ON series.author_id = authors.id INNER JOIN characters ON authors.id = characters.author_id GROUP BY COUNT(characters.species) AS species_count ORDER BY series.title DESC LIMIT 1"
end
